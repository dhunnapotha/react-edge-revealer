...
var EdgeRevealer = React.createClass({
  ...
  _calculateTop: function (styling) {
    if(this.state.show)
      return this.props.container_styling.top || this._getDefaultContainerTopWhenRevealed();
  ...
  render: function () {
    ...
    return (
      <div className="edge-revealer-container" >
        <Waypoint onEnter={this.handleWaypointEnter} onLeave={this.handleWaypointLeave} threshold={this.props.threshold}/>
        <div className="edge-revealer" style={styling}>
          {this.props.children}
        </div>
      </div>
    )
